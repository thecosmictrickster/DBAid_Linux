...
foreach ($Instance in $SqlServer) {
    if ($User -ne 'myuser') {
        $InstanceTag = (Invoke-Sqlcmd -ServerInstance $Instance -Database '_dbaid' -Username $User -Password $Pass -Query 'EXEC [system].[get_instance_tag]')[0]
        $ProcedureList = (Invoke-Sqlcmd -ServerInstance $Instance -Database '_dbaid' -Username $User -Password $Pass -Query "SELECT [proc]=QUOTENAME(SCHEMA_NAME([schema_id])) + N'.' + QUOTENAME([name]) FROM sys.objects WHERE [type] = 'P' AND SCHEMA_NAME([schema_id]) = 'collector'").proc
    } else {
    ...
    if ($User -ne 'myuser') {
        $dt = Invoke-Sqlcmd -ServerInstance $Instance -Database '_dbaid' -Username $User -Password $Pass -Query "EXEC $proc @update_execution_timestamp = $UpdateExecTimestamp" -OutputAs DataTables
    } else {
    ...
    if ($User -ne 'myuser') {
        [string]$secret = (Invoke-Sqlcmd -ServerInstance $Instance -Username $User -Password $Pass -Query "SELECT [value] FROM [_dbaid].[system].[configuration] WHERE [key] = N'COLLECTOR_SECRET'")[0]
    } else {
